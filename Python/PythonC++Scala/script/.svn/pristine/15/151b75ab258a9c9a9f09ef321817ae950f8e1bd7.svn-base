...
def get_item_addon(item_id, sku=True, promo=True, interval=0.4):
    ...
    time.sleep(interval)
    r = requests.get(item_url, timeout=5, cookies=cookies, headers=headers, allow_redirects=True)
    ...
    if sku:
        if re.findall(r'siteId\=2', response_content):
            ...
            if sku:
                sku_j_str = re.findall(r'\{.+\}', sku[0].decode(r.encoding), re.S)[0]
                sku_j_str = _parse_json_str(re.findall(r'\{.+\}', sku_j_str, re.S)[0])
                sku = yaml.load(sku_j_str)
                sku_dict = sku['valItemInfo'].get('skuMap') or {}
        elif re.findall(r'siteId\=\d', response_content):
            sku = re.findall(r'Hub.config.set\(\'sku\'.+?\)', response_content, re.S)
            if sku:
                sku_j_str = re.findall(r'\{.+\}', sku[0].decode(r.encoding), re.S)[0]
                sku_j_str = _parse_json_str(re.findall(r'\{.+\}', sku_j_str, re.S)[0])
                sku = yaml.load(sku_j_str)
    ...
    if promo:
        ...
        if re.findall(r'siteId\=2', response_content):
            # tmall
            init_api = re.findall(r'\"initApi\"\s*\:\s*\"(.+?)\"', response_content)[0]
            has_promo = True
            sku_list = re.findall(r'\"skuId\"\s*\:\s*\"(\d+)\"', response_content, re.S)
            ...
            init_api = re.findall(r'\"apiPromoData\"\s*\:\s*\"(.+?)\"', r.content)
            if init_api:
                has_promo = True
                init_api = init_api[0]
                headers.update({
        ...
        if has_promo:
            init_api = init_api + '&%s' % '&'.join(['%s=%s' % (k, v) for k, v in add_on_params.items()])
            cookies.update(r.cookies)
            time.sleep(interval)
            r2 = requests.get(init_api, timeout=5, headers=headers, cookies=cookies)
            item_meta_content = re.findall(r'\{.+\}', r2.content, re.S)[0]
            item_meta_content = re.sub(r'\t', r'', item_meta_content, flags=re.S)
            item_meta_content = re.sub(r'\n', r'', item_meta_content, flags=re.S)
            item_meta_content = re.sub(r'\r', r'', item_meta_content, flags=re.S)
            item_meta_content = re.sub(r',\s', r',', item_meta_content, flags=re.S)
            item_meta_content = re.sub(r',(\w+)\:', r',"\1":', item_meta_content, flags=re.S)
            item_meta_content = re.sub(r'{(\w+)\:', r'{"\1":', item_meta_content, flags=re.S)
        ...
        if item_detail.get('isSuccess'):
            ...
            # for sku_id in item_detail['sku_list']:
                # price_info = item_detail['defaultModel']['itemPriceResultDO']['priceInfo'][sku_id]
                price_info = item_detail['defaultModel']['itemPriceResultDO']['priceInfo'][sku_key]
