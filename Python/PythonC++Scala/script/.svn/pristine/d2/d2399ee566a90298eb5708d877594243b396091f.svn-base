...
from lib import *
from topbll import saveTopTrade
    ...
    if siteId == '2':
        ...
        while 1:
            ...
            for res in result0[1:]:
                ...
                nick = \
                    re.findall(r'\<span\s+class\=\"tb\-sellnick\"\>\s*(.+?)\s*\<img'
                               , tds[0])
                if len(nick) < 1:
                    nick = \
                        re.findall(r'\<span\s+class\=\"tb\-sellnick\"\>\s*(.+?)\s*\<\/td'
                                   , tds[0])[0]
                else:
                    nick = nick[0]
                ...
                rank = \
                    re.findall(r'\<img\s+src\=\".+?\/(.+?)\.gif\".+?class\=\"rank\"\s*\/\>'
                               , tds[0])
                if len(rank) < 1:
                    rank = None
                else:
                ...
                vipLevel = \
                    re.findall(r'\<a\s+.+?href\=\"http\:\/\/vip\.taobao\.com\"\s+title\=\"(\w\d+).+?\"'
                               , tds[0])
                if len(vipLevel) > 0:
                    vipLevel = vipLevel[0]
                else:
                ...
                metadata.append(re.findall(r'\<td\s+class\=\"tb\-price\"\>(\d+\.*\d*)'
                                , tds[1])[0])
                ...
                promoTitle = \
                    re.findall(r'\<a\s+title\="\s*(\S+?)\s*"\s+class\=\"tb\-promo\"\>'
                               , tds[1])
                if len(promoTitle) > 0:
        ...
        while 1:
            ...
            for res in result0[1:]:
                ...
                nick = \
                    re.findall(r'\<span\s+class\=\"tb\-sellnick\"\>(.+?)\<\/span\>\s*\<'
                               , res)
                if len(nick) > 0:
                    nick = nick[0]
                    for t in re.findall(r'\<.+?\>', nick):
                ...
                price = \
                    re.findall(r'\<td\s+class\=\"tb\-price\"\>(\d+\.*\d*).*?\<\/td\>'
                               , res)[0]
                ...
                promoTitle = \
                    re.findall(r'\<a\s+title\=\"\s*(.+?)\s*\"\s+class\=\"tb\-promo\"\>'
                               , res)
                if len(promoTitle) > 0:
                    promoTitle = promoTitle[0].decode(r.encoding).encode('utf-8'
                            )
                ...
                metadata.append(re.findall(r'\<td\s+class\=\"tb\-amount\"\>\s*(\d+)\s*\<\/td\>'
                                , res)[0])

                # tradeTime

                metadata.append(re.findall(r'\<td\s+class\=\"tb\-time\"\>\s*(.+?)\s*\<\/td\>'
                                , res)[0])

                # sku

                skus = \
                    re.findall(r'\<td\s+class\=\"tb\-sku\"\>.*?\s*(\<p\>.+\<\/p\>)\s*.*?\<\/td\>'
                               , res)[0]
                metadata.append('\t'.join(re.findall(r'\<p\>\s*(.+?)\s*\<\/p\>'
                                , skus)).decode(r.encoding).encode('utf-8'))
                if len(INFOS) > 2:
                    if metadata[7] == INFOS[0][10]:
                        stop = True
                        break
