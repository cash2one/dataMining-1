...
class Connection(object):
    ...
    """
    def __init__(self, host, db, user=None, passwd=None,
                 max_idle_time=7 * 3600, connect_timeout=0,
                 CursorClass=MySQLdb.cursors.SSCursor,
                 time_zone="+8:00", **kwargs):
        self.host = host
        self.database = db
    ...
    def reconnect(self):
        ...
        self.close()
        if self._db_args.get('connection_class'):
            self._db_args.pop('connection_class')
        if self._db_args.get('max_connections'):
            self._db_args.pop('max_connections')
