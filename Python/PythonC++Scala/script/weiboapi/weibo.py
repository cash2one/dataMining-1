...
def _encode_multipart(**kw):
    ...
    for (k, v) in kw.iteritems():
        ...
        if hasattr(v, 'read'):
            ...
            content = v.read()
            data.append('Content-Disposition: form-data; name="%s"; filename="hidden"'
                         % k)
...
class APIClient(object):
    ...
    def get_authorize_url(self, redirect_uri=None, display='default'):
        ...
        if not redirect:
            raise APIError('21305', 'Parameter absent: redirect_uri',
                           'OAuth2 request')
        return '%s%s?%s' % (self.auth_url, 'authorize',
                            _encode_params(client_id=self.client_id,
                            response_type='code', display=display,
...
def main():
    client = APIClient('4166364270', 'e3b32a1fb09c8c5ec0e1dce8cb7e2aa3',
                       'http://192.168.1.185')
    print client.get_authorize_url()
    code = raw_input('code')
    print client.request_access_token(code, 'http://192.168.1.185')
