...
#include <time.h>
/*double integration(double limit,int n){
	double x0 = 0,sum =0.0;
	double gap = limit/n;
	for (int i=0;i<=n-1;i++){
		sum += (func(gap*i)+func(gap*(i+1)))*gap/2.0;
	}
	return sum;
}*/
/*int main(){
...
int main(int argc, char **argv){
	int myid,p,namelen;
	double starttime,endwtime,sum = 0.0,h,xi,mypi,result;
	char processor_name[MPI_MAX_PROCESSOR_NAME];
	MPI::Init(argc,argv);
	myid = MPI::COMM_WORLD.Get_rank();
	p = MPI::COMM_WORLD.Get_size();
	MPI::Get_processor_name(processor_name,namelen);
	cout<<"process"<<myid<<"of"<<p<<"is on"<<processor_name<<endl;
	if (myid==0){
		starttime = MPI::Wtime();
	}
	double n = 10000000,limit =10000;
	double func(double x);
	...
	mypi = h*sum;
	MPI::COMM_WORLD.Reduce(&mypi,&result,1,MPI_DOUBLE, MPI_SUM, 0);
	if (myid == 0){
		endwtime = MPI::Wtime();
	cout<<"result is "<<result<<"\n";
	cout<<"wall clock time = "<<endwtime - starttime<<endl;
	}
	MPI::Finalize();
	return 0;
}